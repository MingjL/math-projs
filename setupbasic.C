...
double **readpicture(FILE *infile, int &width, int &height);
char readchar(FILE *infile, int &num);
int readint(FILE *infile, int numbytes, int &num);
int char2int(char thechar);
int char2int(char *thechar, int numbytes);
void readrgb(FILE *infile, double *rgb, int &num);
void writepicture(FILE *outfile, double **f, int width, int height);
char int2char(unsigned long theint);
void int2char(char *thechar, int theint);
void writechar(FILE *outfile, char thechar);
void writeint(FILE *outfile, int theint, int numbytes);
void writergb(FILE *outfile, double r, double g, double b);
...
// if reading in picture
   FILE *infile = fopen("drapes.bmp","rb");
...
double **readpicture(FILE *infile, int &width, int &height)
{
   ...
   if (bitsperpixel != 24)
   {
      printf("ERROR: this program is only built for 1 byte per rgb, not a total of ");
      printf("%d bits\n",bitsperpixel);
      exit(1);
   }
...
int readint(FILE *infile, int numbytes, int &num)
{
   ...
   for (i = 0; i < numbytes; i++)
      fread(&(temp[i]),1,sizeof(char),infile);
...
char readchar(FILE *infile, int &num)
{
   ...
   fread(&thechar,1,sizeof(char),infile);
   num++;
...
void writechar(FILE *outfile, char thechar)
{
   fwrite(&thechar,sizeof(char),1,outfile);
}
...
void writeint(FILE *outfile, int theint, int numbytes)
{
   ...
   for (i = 0; i < numbytes; i++)
      fwrite(&(temp[i]),sizeof(char),1,outfile);
