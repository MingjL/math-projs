            ...
            int li,ui,lj,uj;
                  ...
                  if(j+1>=height)
                    ...
                    uj = j+1;
                        ...
                        fx = (f[ui][j]-f[li][j])/(2*eps);
                        fy = (f[i][uj]-f[i][lj])/(2*eps);
                        fxx =(f[ui][j]-2*f[i][j]+f[li][j])/(eps*eps);
                        fyy =(f[i][uj]-2*f[i][j]+f[i][lj])/(eps*eps);
                        fxy = (f[ui][uj]-f[ui][lj]-f[li][uj]+f[li][lj])/(4*eps*eps);
                        newf[i][j] = f[i][j]+s*((fxx+fyy)/sqrt(fx*fx+fy*fy+eps*eps)-(fx*(fxx*fx+fxy*fy) +fy*(fxy*fx+fyy*fy))/sqrt(pow(fx*fx+fy*fy+eps*eps,3))); 
                        //printf("%lf\n", newf[i][j]);
                 // }
            }
      }
