...
int getmax(double **f, char x, int width, int height);
//MAIN BODY HERE
...
// if reading in picture
   ifstream infile("octopus.bmp", ios::in | ios::binary);
   f = readpicture(infile,width,height);
   infile.close();
   
   setintensity(8,f,width,height);
// if creating picture
//   width = 100;
//   height = 100;
...
void setintensity( int inten, double **f,int width, int height){
      ...
      for (int i = 0; i<width; i++){
            for (int j = 0; j<height;j++){
                  for(int k = 0; k<inten-1;k++){
                        double low = intensity[k];
                        double high = intensity[k+1];
                        

                     if(f[i][j]>=low &&f[i][j]<high){
                        if (f[i][j] <(low+high)/double(2)){
                              f[i][j] = low;
                              intencount[k]++;
     ...
     for (int i = 0; i<inten; i++){
           printf("intensity: %f, count: %d\n",intensity[i],intencount[i]);
     }
