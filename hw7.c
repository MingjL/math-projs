...
double ** heatflow(double **f, int width, int height, int step,double h);
//MAIN BODY HERE
 ...
 int main()
{
   int width;
   int height;
   double **f,**newf,**f0,**gof, **gof0, **ngof0,**ef;
   double x, y;
   int i, j;

// if reading in picture
ifstream infile("zebrablurry.bmp", ios::in | ios::binary);
...
//f = setimg(width,height,-1,1,-1,1
   ...
   gof = matrix(width,height);
   gof0 = matrix(width,height);
   ngof0 = matrix(width,height);
   f0 = matrix(width,height);
   ef = matrix(width,height);
  ...
  double  s = (pow(h,2)*eps)/(4+lamb*h*h*eps);

  f0 = heatflow(f,width,height,heatstep,h);
  printf("17,17,%lf\n",f0[17][17]);
  printf("\n");
   ofstream outfile1("initial.bmp", ios::out | ios::binary);
   writepicture(outfile1,f0,width,height);
   outfile1.close();
  printf("%d,%d\n",width,height);
   while (step<=500)
   {
      gof = heatflow(f,width,height,heatstep,h);
      // printf("gof: %lf\n",gof[17][17]);
      for (int l = 0; l<width;l++){
                  for (int k = 0; k<height;k++){
                         gof0[l][k] = gof[l][k] - f0[l][k];
                  }
       }
                   
      ngof0 = heatflow(gof0,width,height,heatstep,h);
      for (int i = 0; i<width;i++){
            for (int j = 0; j<height;j++){
            //newf[i][j] = f[i][j];
                //  if(D[i][j] ==1){
                  ...
                  if(j+1>=height)
                    ...
                    uj = j+1;
                        ...
                        fxy = (f[ui][uj]-f[ui][lj]-f[li][uj]+f[li][lj])/(4*h*h); 
                         newf[i][j] = f[i][j]+s*((fxx+fyy)/sqrt(fx*fx+fy*fy+eps*eps)-(fx*(fxx*fx+fxy*fy) +fy*(fxy*fx+fyy*fy))/sqrt(pow(fx*fx+fy*fy+eps*eps,3)) - 2*lamb*ngof0[i][j]);
      ...
      for(int i = 0; i<width; i++){
            for (int j = 0; j <height;j++){ 
                  ...
                  if(j+1>=height)
                   ...
                   newf[i][j] = (ef[ui][j]+ef[li][j]+ef[i][uj]+ef[i][lj])/4; 
            }
      }
      //themax = 0;
