...
//   YOUR FUNCTIONS HERE
void setintensity(int inten, double **f,int width,int height);
...
// if reading in picture
   ifstream infile("gallery.bmp", ios::in | ios::binary);
   f = readpicture(infile,width,height);
   infile.close();
   
   setintensity(4,f,width,height);
// if creating picture
//   width = 100;
//   height = 100;
...
//**************************************************************************
void setintensity( int inten, double **f,int width, int height){
      double min = getmax(f,'m',width, height);
      double max = getmax(f, 'M',width, height); 
       double h = (max-min)/(double)(inten-1);
       double *intensity = new double[inten];
       int *intencount = new int[inten];
       double *GoC = new double[inten];
       
       for (int i = 0; i<inten;i++){
             intensity[i]=min+h*(double)(i-1);
       }
       printf("inten: %lf\n", intensity[2]);
      for (int i = 0; i<inten; i++){
            GoC[i]=(CH(f,intensity[i],height,width)-CH(f,min,height, width))/(CH(f,max,height, width)-CH(f, min,height,width));
      }
      printf("ch %f\n", GoC[2]);
       for(int i = 0; i<width;i++){
             for (int j = 0; j<height;j++){
                   for (int k = 0;k<inten-1;k++){
                         if (f[i][j]>=intensity[k]&&f[i][j]<=intensity[k+1]){
                               f[i][j] = (f[i][j]-intensity[k])*(GoC[k+1]-GoC[k])/(intensity[k+1]-intensity[k]) + GoC[k];
                         }
                   }
             }
       }
